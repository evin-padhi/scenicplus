...
search_space_fname = '/oak/stanford/groups/smontgom/epadhi/projects/trisomy_multiomics/ts21_18_ctrl_data/scenic_analyses/scenicplus_pipeline/output/search_space.tsv'
multiome_mudata_fname = '/oak/stanford/groups/smontgom/epadhi/projects/trisomy_multiomics/ts21_18_ctrl_data/scenic_analyses/scenicplus_pipeline/output/ACC_GEX.h5mu' 

mdata = mudata.read(multiome_mudata_fname)
search_space = pd.read_csv(search_space_fname,sep = '\t')
cell_metadata = mdata.obs

############ PREPROCESS DATA


"exprs","atac_condition","atac","condition","percent_mito","log_nUMI","sample"
covariates = pd.get_dummies(mdata.obs[['scATAC:condition','scATAC:dataset']]).astype(int)
...
adj.to_csv(
        'conditional_region_to_gene.tsv',
...
region_to_gene_importances_filtered = {
    key: series[series.index.str.contains('chr')]
    for key, series in region_to_gene_importances.items()
}
...
with open('region_to_gene_conditional_importances.pkl',"wb") as f:
    pickle.dump(region_to_gene_importances,f)
with open('region_to_gene_conditional_correlation.pkl',"wb") as f:
    pickle.dump(region_to_gene_correlation,f)




result_df = pd.concat([pd.DataFrame(data={'target': gene,
                                                'region': region_to_gene_importances_filtered[gene].index.to_list(),
                                                'importance': region_to_gene_importances_filtered[gene].to_list(),
                                                'rho': region_to_gene_correlation[gene].loc[
                                                    region_to_gene_importances_filtered[gene].index.to_list()].to_list()})
                            for gene in region_to_gene_importances_filtered.keys()
                            ]
                            )
...
result_df.to_csv('region_to_gene_conditional.tsv',sep = '\t')
...
def _score_regions_to_single_gene(
    ...
    if regressor_type in SKLEARN_REGRESSOR_FACTORY.keys():
        ...
        predictor_names = np.concatenate((covariates.columns,region_names))

        # predictor_names = covariates.columns.append(region_names)
        merged_input = np.array([np.concatenate([arr, info]) for arr, info in zip(covariates.values, acc)])
        
        # fit model
        fitted_model = arboreto_core.fit_model(regressor_type=regressor_type,
                                               regressor_kwargs=regressor_kwargs,
                                               # tf_matrix=acc,
                                               tf_matrix = merged_input,
        ...
        return gene_name, pd.Series(feature_importance, index=predictor_names)
...
def _get_acc_idx_per_gene(
    ...
    # region_names = np.array([x.replace(':','-') for x in search_space['Name']])
    region_names = search_space["Name"].to_numpy()
...
def _score_regions_to_genes(
    ...
    scplus_region_names = np.array([x.replace('-',":",1) for x in df_acc_mtx.columns])
...
def calculate_regions_to_genes_relationships(
    ...
    result_df = pd.concat([pd.DataFrame(data={'target': gene0
